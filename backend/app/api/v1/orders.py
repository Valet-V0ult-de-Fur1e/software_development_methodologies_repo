...
async def get_my_orders(
    ...
    order_repo = OrderRepository(session)
    orders = await order_repo.get_by_user(current_user.id)
    return [OrderResponse.from_orm(o) for o in orders]
...
async def get_order(
    ...
    if order.user_id != current_user.id and current_user.role not in [UserRole.admin, UserRole.manager]:
...
async def create_order(
    ...
    if order_create.user_id != current_user.id:
